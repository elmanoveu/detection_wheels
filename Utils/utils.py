...
def draw_annotated_box(image, box_list_list, label_list, color_list):
    ...
    return image
def convert_predictions(frame_coord):
    wheel_frame = defaultdict(list)
    for n_frame,frame in enumerate(frame_coord):
        if frame[4]==4:
            wheel_frame[1].append(frame)     
    new_frame=defaultdict(list)
    for f,n in enumerate(wheel_frame):
        if len(wheel_frame[n])>1:
            Square_1=abs(wheel_frame[n][0][1]-wheel_frame[n][0][3])*abs(wheel_frame[n][0][0]-wheel_frame[n][0][2])
            Square_2=abs(wheel_frame[n][1][1]-wheel_frame[n][1][3])*abs(wheel_frame[n][1][0]-wheel_frame[n][1][2])
            if Square_1>Square_2:
                new_frame[f+1].append(wheel_frame[n][0])
            else:
                new_frame[f+1].append(wheel_frame[n][1])
        else:
            new_frame[f+1].extend(wheel_frame[n])
    return new_frame
